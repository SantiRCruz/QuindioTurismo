...
class DBManager(context: Context) {
    ...
    fun listar(nombreTabla: String): MutableList<Informacion> {
        ...
        if (result!!.moveToFirst())
            do {
                ...
                informacion.nombre =
                    result!!.getString(result.getColumnIndex(Constantes.TABLE_COLUMN_1))
                informacion.descripcionCorta =
                    result!!.getString(result.getColumnIndex(Constantes.TABLE_COLUMN_2))
                informacion.ubicacion =
                    result!!.getString(result.getColumnIndex(Constantes.TABLE_COLUMN_3))
                informacion.descripcion =
                    result!!.getString(result.getColumnIndex(Constantes.TABLE_COLUMN_4))
    ...
    fun consultarDetalle(nombreTabla: String, nombreItem: String): MutableList<Informacion> {
        var lista: MutableList<Informacion> = ArrayList()
        openDbWR()
        var result = db?.rawQuery(Constantes.CONSULTA + nombreTabla + " WHERE " + Constantes.TABLE_COLUMN_1 + "=?", arrayOf(nombreItem))
        if (result!!.moveToFirst())
            do {
                var informacion = Informacion()
